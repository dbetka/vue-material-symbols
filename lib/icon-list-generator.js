...
function generateIconsDictionary (icons) {
  return new Promise(resolve => {
    const definitions = icons.map(icon => `'${icon.name}':'${icon.name}';`)
...
function generateTypesDictionary (types) {
  return new Promise(resolve => {
    const definitions = types.map(type => {
      ...
      return `  '${shortName.toLowerCase()}': '${shortName.toLowerCase()}'\n`
    })
  ...
  return new Promise((resolve, reject) => {
    fetch(inputListOfIcons)
      ...
      .then(googleJSON => {
        ...
        metadata.families = metadata.families.filter(type => type.includes('Symbols'))
